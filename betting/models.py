...
class Apuesta(models.Model):
    ...
    @transition(
        field=estado,
        source=EstadoApuesta.ACEPTADA,
        target=EstadoApuesta.CASH_OUT,
    )
    def marcar_cash_out(self):
        """
        Transición FSM de cash-out anticipado.

        El campo `estado` tiene protected=True: NUNCA asignar
        apuesta.estado = ... ni confiar en refresh_from_db() sin excluir
        `estado`; usar siempre este método seguido de save().
        """
        pass
